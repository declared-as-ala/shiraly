      ...
      <header className="sticky top-0 z-30 border-b border-ink-200 bg-sand-50/95 backdrop-blur">
        <div className="container-shop flex items-center gap-3 py-3 sm:gap-4">
          ...
          <div className="flex flex-1 items-center justify-end gap-2">
            ...
            <LanguageSwitcher />
            <Link href="/#products" className="grid min-h-11 min-w-11 place-items-center rounded-lg text-ink-900 hover:bg-ink-100" aria-label={t.nav.search}>
              <Search size={20} />
            </Link>
        ...
        {open && (
          <nav className="border-t border-ink-200 bg-sand-50 p-3 lg:hidden">
            {[...navLinks, { href: '/#products', label: t.nav.collections }].map((l) => (
              <Link key={l.href} href={l.href} onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2 font-semibold text-ink-900 hover:bg-ink-100">
                {l.label}
              </Link>
            ))}
            {categories.length > 0 && (
              <>
                ...
                {categories.map((c) => (
                  <Link key={c.slug} href={`/categorie/${c.slug}`} onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2 font-semibold text-ink-900 hover:bg-ink-100">{c.name}</Link>
                ))}
              </>
            )}
          </nav>
